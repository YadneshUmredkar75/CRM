...
const courseAPI = {
  ...
  updateStudent: async (courseId, studentId, studentData) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`http://localhost:8080/api/courses/${courseId}/students/${studentId}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
  ...
  updateReminder: async (courseId, studentId, reminderMessage) => {
    const token = localStorage.getItem('adminToken');
    const response = await fetch(`http://localhost:8080/api/courses/${courseId}/students/${studentId}/reminder`, {
      method: 'PATCH',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-6">
        ...
        {error.includes('server') && (
          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800">
            ...
            <p>• Verify API endpoint: http://localhost:8080/api/courses</p>
